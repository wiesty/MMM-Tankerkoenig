Module.register("MMM-Tankerkoenig", {
  ...
  start: function () {
    this.loaded = false;
    this.prices = {};
    this.url = `https://creativecommons.tankerkoenig.de/json/prices.php?apikey=${this.config.apiKey}&ids=${Object.keys(this.config.stationNames).join(",")}`;
    this.getData();
    setInterval(() => {
      this.getData();
...
getDom: function () {
  ...
  for (var fuelType of this.config.fuelTypes) {
